...
function generateTimetable(){
  ...
  const dayChecks = [...document.querySelectorAll('.days input[type="checkbox"]')];
  ...
  for(let w=1; w<=4; w++){
    ...
    const th0 = document.createElement('th'); th0.textContent = ''; th0.className='label';
    headRow.appendChild(th0);
    days.forEach(d => {
      const th = document.createElement('th'); th.textContent = d; th.className='label';
    ...
    const grid = Array.from({length: slots.length}, () => Array.from({length: days.length}, () => ''));

    // Compose class list according to week
    const classList = [];
    for(let i=1; i<=num5; i++) classList.push(`5세 ${i}반 (카니)`);
    for(let i=1; i<=num6; i++) classList.push(`6세 ${i}반 (${instr6[w-1]})`);
    for(let i=1; i<=num7; i++) classList.push(`7세 ${i}반 (${instr7[w-1]})`);
    ...
    for(let r=0; r<slots.length; r++){
      ...
      const label = document.createElement('td'); label.textContent = slots[r]; label.className='label';
      tr.appendChild(label);
      for(let c=0; c<days.length; c++){
        const td = document.createElement('td'); td.textContent = grid[r][c] || '';
        tr.appendChild(td);
      }
    ...
    if (!alerted){
      for(let c=0; c<days.length; c++){
        ...
        if(cnt >= 3){
          alert('하루 수업 시수가 3회 이상입니다. 카니가 1세트 이상 필요한지 확인이 필요합니다.');
          alerted = true;
          break;
        }
      }
